...
1. **抽离数据逻辑层：自定义 Hook (`useUsers.ts`)**
   ...
   - 进一步抽象底层通用泛型 Hook `useFetch<T>`，实现基础设施与领域业务分离；
   - 让外部只需要一行：`const { users, loading, error, refetch, removeUser } = useUsers();` 就能获得全套数据能力（类比注入 Spring `@Service`）。
...
3. **拆分独立展示单元：`UserCard.tsx`**
   - 接收 Props：`user: ApiUser` 与事件回调 `onDelete: (id: number) => void`；
   - 作为纯函数组件（Dumb Component），只负责渲染单个用户的精美卡片并触发删除事件。
...
- **自定义 Hook 本质**：
  - 必须以 `use` 开头（如 `useUsers` / `useFetch`）；
...
- [x] 成功封装 `useUsers` 自定义 Hook，数据请求与刷新逻辑从 UI 中解耦；
- [x] 成功拆分 `UserSearchBar` 与 `UserCard` 子组件，Props 契约严谨（带 TS 类型）；
- [x] 父子通信流畅，搜索过滤、刷新、Loading、Error 表现与 TASK-004 一致；
- [x] 拓展完成 `onDelete` 事件回调联动与 `useFetch<T>` 泛型抽象；
