...
> **状态**：✅ **已通关 / 已归档（2026-09-19）**｜核心 DoD ①~⑧ + ⑪ 全项交付；进阶项 ⑨⑩ 未做（保留为后续练习）；口试免测，参考答案见 [`../notes/final-exam.md`](../notes/final-exam.md)。
...
- [x] ① `adminUserApi.ts` 实现 4 个函数（泛型齐全、URL 用相对子路径、不手写 Authorization 头）
- [x] ② `AdminUsersPage` 完成：分页 + 关键字搜索（**防抖生效**：连续输入只打一次请求）+ 四态渲染
- [x] ③ 角色感知：`guest` 登录时「新增 / 编辑 / 删除」入口消失；直接调接口返回 **403 中文提示**
- [x] ④ 新增 / 编辑弹窗可用：受控表单 + `submitting` 禁用 + 错误提示 + 成功后刷新列表
- [x] ⑤ 删除成功且**末页被删空时页码自动回退**（不出现空列表 + 越界页码）
- [x] ⑥ `UserDetailPage` 完成：`useParams` 取 id（含 `string | undefined` 收窄）+ 三态 + 404 中文提示
- [x] ⑦ `DashboardPage` 完成：展示当前用户 + 用户总数（用分页接口的 `total`，不拉全量）
- [x] ⑧ 未登录访问任意管理页 → 跳登录；刷新浏览器保持登录（回归 TASK-008）
- [ ] ⑨ （进阶）**全局 401 处理**：token 失效后任意请求都能触发“清 token + 跳登录”（不是每个页面各写一遍）—— **本次未实现，保留为后续练习**（设计提示见 [`../notes/final-exam.md`](../notes/final-exam.md) Q3）
- [ ] ⑩ （进阶）**请求取消**：快速切换参数时，在途请求被 `AbortController` 真正取消（Network 面板可见 `(canceled)`）—— **本次未实现，保留为后续练习**（当前用 `active` 门禁丢弃过期响应）
- [x] ⑪ **生产构建与部署**：`npm run build` 成功 + `npm run preview` 可跑通；写出 Nginx 静态托管 + `/api` 反向代理配置样例（归档到 [`docx/notes/deploy.md`](../notes/deploy.md)）
- [x] ⑫ 通过教练终审与结业口试（跨任务综合考察）—— **口试免测**；8 题参考答案与自测评分表归档于 [`docx/notes/final-exam.md`](../notes/final-exam.md)

### 🚀 结业加分项（选做）
- [ ] 用户列表支持排序（按 id / 姓名），并处理"排序变化 → 页码归 1"
- [ ] 表格行的加载骨架（skeleton）替代整块 loading，减少布局跳动
- [x] 表单校验：邮箱格式、必填项（提交前拦截，错误定位到字段）
- [ ] 404 页面提供"返回仪表盘"按钮
